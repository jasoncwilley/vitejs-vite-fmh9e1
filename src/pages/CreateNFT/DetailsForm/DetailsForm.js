...
const transaction = Arweave.createTransaction()
 export const addMeta = function addMeta(name,value){
   transaction.addtag(name.toString(), value.toString())
 };

export async function ARweaveHandler(){
  ...
  let arKey = process.env.ARWEAVE_KEY.toString();

  const arweaveKey = JSON.parse(arKey);

  const arweaveWallet = await arweaveconnection.wallets.jwkToAddress(arweaveKey);

  const arweaveWalletBallance = await arweaveconnection.wallets.getBalance(arweaveWallet)
    var balance = arweaveWalletBallance().toString()
    console.log(balance)

  var buf ="";
   let transaction = await Arweave.createTransaction({data: buf}, arweaveKey);

    addMeta('Content-Type', 'image/png');
};
...
export const DetailsForm = (props) => {
  ...
  const pic2Base64 = function(){
    var image = document.querySelector('#image-preview');
    var base64String = "";
    var uploadButton = document.getElementById("upload-Button");
    image.addEventListener("loading", function () {
    ...
    uploadButton.addEventListener("click", function () {
      var imgElement = document.getElementById("imageBase64");
      imgElement.src = "data:image/png;base64,".concat(base64String);
    });
  }
  const handlePhoto = (e) => {
    e.preventDefault();
        ...
        <>
          <div className="box" style={{ display: "flex", flexDirection: "column" }}>
            <Form className="form">
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                <button variant="primary" id="upload-Button" type="button" onClick={ARweaveHandler} className="browser-btn">
                Upload 2 ARweave
                 </button>
              </div>
             </Form>
            <div classNae="lines"></div>
          </div>
        </>
      )}</>);
};
